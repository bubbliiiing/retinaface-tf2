        ...
        while True:
            ...
            for i, image_path in enumerate(self.imgs_path):  
                ...
                if len(regression_targets) == self.batch_size:
                    yield preprocess_input(np.array(inputs)), np.array(regression_targets,dtype=np.float32), np.array(classification_targets,dtype=np.float32), np.array(landms_targets,dtype=np.float32)
                    inputs                  = []
                    regression_targets      = []
                    classification_targets  = []
                    landms_targets          = []
